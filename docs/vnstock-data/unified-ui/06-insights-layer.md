...
| `filter()` | `limit`, `filters` | Dữ liệu screener theo bộ lọc hoặc toàn thị trường | DataFrame |

**Parameters:**
- `lang` (str): Ngôn ngữ ('vi' hoặc 'en'). Mặc định 'vi' (dùng cho hàm `criteria()`).
- `limit` (int): Số lượng bản ghi tối đa. Mặc định 2000 (dùng cho hàm `filter()`).
- `filters` (List[Dict], optional): Danh sách cấu hình bộ lọc tùy chỉnh. Trùng định dạng list filter của VCI. Nếu không truyền, hệ thống sẽ gọi bộ lọc toàn thị trường.

#### Bảng Tham Chiếu Cấu Trúc Bộ Lọc (`filters`)

Cấu trúc JSON cơ bản của tham số `filters` (kiểu dữ liệu `List[Dict]`) như sau:
```json
[
    {
...
Do số lượng tiêu chí lọc rất lớn, bạn nên yêu cầu AI viết cấu trúc payload JSON cho `filters` bằng cách cung cấp danh sách cột từ hàm `criteria()`.

**Prompt mẫu:**
> "Tôi đang dùng thư viện vnstock_data, hàm `ins.screener().filter(filters=...)` cho phép truyền danh sách các điều kiện để lọc cổ phiếu. Dưới đây là danh sách tiêu chí lấy từ hàm `criteria()`: 
> 
> [Copy/paste kết quả từ hàm `ins.screener().criteria().to_csv()` hoặc `to_json()` vào đây]. 
> 
> Hãy soạn cho tôi danh sách các dictionary Python (kiểu JSON list) để lọc các cổ phiếu thuộc sàn HSX, có ROE > 15%, P/E < 10, và khối lượng giao dịch trung bình 30 ngày > 1,000,000."
...
if not df_filtered.empty:
    print(df_filtered[['symbol', 'pe', 'roe']].head())

# Output mẫu:
# Cổ phiếu HSX, P/E < 10, ROE > 15: 90
#   symbol        pe        roe
# 0    ABT  4.473103  25.604934
# 1    ACB  7.420714  17.496054
# 2    ADP  7.493023  25.324429
# 3    ANT  5.978100  34.143393
# 4    ANV  6.087596  30.253528
...
# Giả sử '1000' là "Tài nguyên Cơ bản / Hóa chất" 
sector_code = "1000" 
sector_filters = [
    {
        "name": "sectorLv1",
...
df_sector = ins.screener().filter(filters=sector_filters)
print(f"\nCổ phiếu thuộc nhóm ngành {sector_code}: {len(df_sector)}")
if not df_sector.empty:
    print(df_sector[['symbol', 'exchange', 'vi_sector']].head())

# Output mẫu:
# Cổ phiếu thuộc nhóm ngành 1000: 173
#   symbol exchange          vi_sector
# 0    AAA     HOSE           Hóa chất
# 1    AAH    UPCOM  Tài nguyên Cơ bản
# 2    ABS     HOSE           Hóa chất
# 3    ACG     HOSE  Tài nguyên Cơ bản
# 4    ACM    UPCOM  Tài nguyên Cơ bản
...
# Merge để tìm cơ hội (Cột 'symbol' đã được chuẩn hóa ở cả 2 API)
opportunity = losers.merge(screener, on='symbol', how='inner')
print(opportunity[['symbol', 'pe', 'roe']].head())
```
